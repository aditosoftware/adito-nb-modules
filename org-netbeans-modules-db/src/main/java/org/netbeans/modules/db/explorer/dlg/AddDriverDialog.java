...
public final class AddDriverDialog extends javax.swing.JPanel
{
  ...
  private void drvClassComboBoxActionPerformed(java.awt.event.ActionEvent evt)
  {//GEN-FIRST:event_drvClassComboBoxActionPerformed
    if (!customizer)
    {
  ...
  private void drvListValueChanged(javax.swing.event.ListSelectionEvent evt)
  {//GEN-FIRST:event_drvListValueChanged
    updateState();
  ...
  private void findDriverClass()
  {
    ...
    for (int i = 0; i < drvs.size(); i++)
    {
      try
      {
        ...
        {
          if (jf.getEntry(drivers[j].replace('.', '/') + ".class") != null)
          {  //NOI18N
