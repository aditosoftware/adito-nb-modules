...
//        FormLoaderSettings.getPreferences().addPreferenceChangeListener(settingsListener);
    ...
    replicator = new VisualReplicator(true, FormUtils.getViewConverters()/*,
            FormEditor.getBindingSupport(formModel)*/); // A
...
//            }
  ...
  private void switchNodeInLookup(boolean includeDataNodeLookup)
  {
    ...
    {
      lookups[index] = includeDataNodeLookup && formEditor.getFormDataObject().isValid() // A
...
//    }
  ...
  {
    if (replicator != null)
    { // not on un-initialized designer
  ...
  {
    if (formModel == null)
    { // the form can be closed just after opened, before this gets called (#70439)
      return;
    }
  ...
  {
    if (formModel == null)
    { // the form can be closed just after opened, before this gets called (#70439, #240027)
      return;
    }
    Dimension size = null;
  ...
  private void syncNodesFromComponents()
  {
    ...
    {
      if (c.getNodeReference() != null)
      { // issue 126192 workaround
...
//                    createConnection(connectionSource, connectionTarget); // A
        }
//                resetConnection();
        toggleSelectionMode();
      }
  ...
  public void startInPlaceEditing(RADComponent metacomp)
  {
    ...
    if (JTabbedPane.class.isAssignableFrom(metacomp.getBeanClass()))
    {
      ...
      {
        if (props[i].getName().equals("TabConstraints.tabTitle"))
        { // NOI18N
  ...
  public void loadingComplete()
  {
    ...
    if (active)
    { // focus HandleLayer, need to invoke later - not in hierarchy yet
      ...
      else
      {
        ...
        for (int i = 0; i < events.length; i++)
        {
          ...
          if ((ev.getChangeType() == FormModelEvent.COMPONENT_ADDED)
              ...
              /*|| (ev.getChangeType() == FormModelEvent.BINDING_PROPERTY_CHANGED)*/) // A
          {
            lafBlock = true;
            break;
          }
    ...
    public void run()
    {
      ...
      for (int i = 0; i < events.length; i++)
      {
        ...
        else if (type == FormModelEvent.COMPONENT_PROPERTY_CHANGED)
        {
          ...
          { // bug #220513, don't update anything e.g. for a11y properties
            replicator.updateComponentProperty(eventProperty);
            //updateConnectedProperties(eventProperty, eventComponent); // A
