...
public class FormEditor
{
  ...
  public boolean loadForm()
  {
    if (!formLoaded)
    {
      ...
      { // loading must be done in AWT event dispatch thread
        try
        {
      ...
      throws PersistenceException
  {
    ...
    if (!it.hasNext())
    { // there's no PersistenceManager available
    ...
    { // some errors occurred when recognizing the form file format
      Throwable annotateT = null;
      int n = persistenceErrors.size();
      if (n == 1)
      { // just one exception occurred
 ...
 * @return BindingDesignSupport of given form *//*
  ...
  public static boolean updateProjectForNaturalLayout(FormModel formModel)
  {
    ...
    if (formEditor != null
        ...
        && !ClassPathUtils.isOnClassPath(formEditor.getFormDataObject().getPrimaryFile(), "org.jdesktop.layout.GroupLayout"))
    { // NOI18N
      try
      {
