...
public class HandleLayer extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener
{
  ...
  protected void paintComponent(Graphics g)
  {
    ...
    { // just paint the selection of selected components
    ...
    if (buttonGroups != null)
    {
      ...
      for (RADComponent metacomp : getFormModel().getComponentList())
      {
        ...
        if (buttonGroup != null)
        {
          java.util.List<AbstractButton> members = buttonGroups.get(buttonGroup);
          if (members != null)
          { // Can be null if no button from this group is selected
...
//                    }
    ...
    else if (keyCode == KeyEvent.VK_ESCAPE)
    {
      ...
      if (endDragging(null))
      {
        ...
        || ((keyCode == KeyEvent.VK_F10) && e.isShiftDown()))
    { // Shift F10 invokes context menu
      Point p = null;
      java.util.List selected = formDesigner.getSelectedComponents();
      if (selected.size() > 0)
      {
...
/*    private RADComponent[] getComponentsIntervalToSelect(
  ...
  private void processMouseClickInLayout(RADComponent metacomp, MouseEvent e)
  {
    ...
    AditoHandleLayer.mouseClick(formDesigner, metacomp, e);
  ...
  private RADVisualComponent[] getComponentsToDrag()
  {
    ...
    for (Iterator it = selectedComps.iterator(); it.hasNext(); )
    {
      ...
      if (!subcontained)
      {
        ...
        { // Otherwise, if trying to drag something in scrollpane,
    ...
    { // new layout may impose more limitation
...
//                repaint();
  ...
  private boolean shouldHighlightPanel(JPanel panel, RADComponent radPanel)
  {
    if (panel != null)
    {
      ...
      { // Maybe we should highlight also panels with EmptyBorder
        return false;
      }
...
//        }
  ...
  private int getComponentResizable(RADVisualComponent metacomp)
  {
    ...
    { // old layout support
      Container cont = (Container) formDesigner.getComponent(metacont);
      if (cont != null)
      { // might be null if component just enclosed in container not yet cloned
  ...
  public void mousePressed(MouseEvent e)
  {
    ...
    if (rightMousePressed || popup)
    {
      ...
      else if (endDragging(null))
      { // there was dragging, now canceled
        repaint();
      }
      else
      {
        ...
        else if (!leftMouseDown)
        {
          ...
          if (popup)
          { // context menu on Mac on Ctrl+left mouse press, on Linux on right mouse press
            showContextMenu(p);
          }
        }
  ...
  public void mouseDragged(MouseEvent e)
  {
    ...
    if (!draggingEnded && !anyDragger() && lastLeftMousePoint != null)
    { // no dragging yet
      if (!viewOnly
  ...
  private abstract class ComponentDrag
  {
    ...
    void move(Point p, int modifiers)
    {
      ...
      if (!lockDimension && fixedDimension)
      { // CTRL released in new layout
        fixedTarget = null;
  ...
  private static void paintDraggedComponent(Component comp, Graphics g, float opacity)
  {
    ...
    { // inspired by bug #62041 (JProgressBar bug #5035852)
  ...
  private class ExistingComponentDrag extends ComponentDrag
  {
    ...
    void init()
    {
      ...
      { // dragging started in the old layout support
  ...
  private class NewComponentDrag extends ComponentDrag
  {
    ...
    { // can be re-inited
      ...
      if (precreated != null)
      {
        ...
        { // continuing adding - new instance of the same component
        ...
        { // repeated adding of the same component type, reuse last bounds
      ...
      { // It's a non visual component, don't trying to instantiate
      ...
      { // The corresponding class cannot be loaded, or the component instantiated. Cancel the drag.
        showingComponents = null;
    ...
    boolean end(Point p, int modifiers)
    {
      ...
      if (p != null)
      {
        ...
        { // there is a precreated visual component
          ...
          if (layoutComponent != null && getLayoutModel() != null)
          { // Some beans don't have layout
            createLayoutUndoableEdit();
            boolean autoUndo = true;
            try
            {
      ...
      try
      {
        ...
        if (item != null)
        {
          ...
          { // STRIPPED
            ...
            { // failed to create instance of the component (already reported)
              if (formDesigner.getDesignerMode() == FormDesigner.MODE_ADD)
              {
                formDesigner.toggleSelectionMode(); // also calls endDragging(null)
              }
          ...
          { // can't use class from default package
            dtde.rejectDrag();
          }
        }
        else
        { // could not load the dragged class, or dragging something unknown
          dtde.rejectDrag();
        }
      }
      catch (Exception ex)
